...
export function ChatView({
  ...
  useEffect(() => {
    ...
    // Listen for chat termination (case closure)
    socket.on("chat_terminated", (data: { reason: string }) => {
      console.log("Chat terminated:", data);
      if (data.reason === "case_closed") {
          ...
          {activeChat ? (
            <>
              ...
              <div className="p-4 border-t">
                {chatArchived ? (
                  <div className="flex flex-col items-center justify-center py-5 bg-slate-50/80 rounded-xl border border-slate-200/60">
                    ...
                    <p className="text-sm font-semibold text-slate-600">Conversation Archived</p>
                    <p className="text-xs text-slate-400 mt-0.5">
                      This matter has been closed and no longer accepts new messages.
                    </p>
                  </div>
                ) : (
                <form
