...
export function getUserFromRequest(request: NextRequest) {
  ...
  return { role: userRole, token: tokenCookie.value };
}
...
export function checkRouteAccess(pathname: string, userRole: string | null): boolean {
  ...
  if (lawyerRoutes.some(route => pathname.startsWith(route)) && userRole !== 'lawyer') {
    return false;
  }
  
  if (clientRoutes.some(route => pathname.startsWith(route)) && userRole !== 'client' && userRole !== 'user') {
    return false;
  }
